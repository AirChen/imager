...
namespace AIHoloImager
{
    inline namespace {
        ...
        public:
            tformat(std::string_view fmt) : fmt_(fmt) {}
            
            template<typename T>
            tformat& arg(T&& value) {
                if constexpr (std::is_convertible_v<T, std::string_view>) {
                    // 对于字符串类型，避免创建临时 string
                    args_.emplace_back(value);
                } else {
                    // 对于其他类型，使用 stringstream 转换
            ...
            std::string str() const {
                std::string result;
                result.reserve(fmt_.length() + args_.size() * 16); // 预分配空间
                
                size_t arg_index = 0;
                size_t pos = 0;
                
                while (pos < fmt_.length()) {
                    if (fmt_[pos] == '{' && pos + 1 < fmt_.length() && fmt_[pos + 1] == '}') {
                        // 添加参数
                        if (arg_index < args_.size()) {
                            result += args_[arg_index++];
                        } else {
                            result += "{}";
                        }
                        pos += 2;
        ...
        std::string format(std::string_view fmt, Args&&... args) {
            tformat formatter(fmt);
            (formatter.arg(std::forward<Args>(args)), ...);
    ...
    private:
        std::string ToOpenMvs(const StructureFromMotion::Result& sfm_input)
        {
            ...
            for (size_t i = 0; i < sfm_input.views.size(); ++i)
            {
                ...
                const auto image_path = out_images_dir / format("{}.jpg", i);
            ...
            {
                throw std::runtime_error(format("Fail to save the file {}", out_mvs_file.string()));
            }
        ...
        std::string PointCloudDensification(const std::string& mvs_name)
        {
            ...
            const std::string cmd = format("{} {}.mvs -o {}.mvs --resolution-level 1 --number-views 8 --process-priority 0 "
                                                "--remove-dmaps 1 --ignore-mask-label 0 -w {}",
                (exe_dir_ / "DensifyPointCloud").string(), mvs_name, output_mvs_name, working_dir_.string());
            const int ret = std::system(cmd.c_str());
            if (ret != 0)
            {
                throw std::runtime_error(format("DensifyPointCloud fails with {}", ret));
            }
        ...
        std::string RoughMeshReconstruction(const std::string& mvs_name)
        {
            ...
            if (ret != 0)
            {
                throw std::runtime_error(format("ReconstructMesh fails with {}", ret));
            }
        ...
        std::string MeshRefinement(const std::string& mvs_name, const std::string& mesh_name)
        {
            ...
            const std::string cmd = format("{} {}.mvs -m {}.ply -o {}.ply --scales 1 --gradient-step 25.05 --cuda-device -1 -w {}",
                (exe_dir_ / "RefineMesh").string(), mvs_name, mesh_name, output_mesh_name, working_dir_.string());
            const int ret = std::system(cmd.c_str());
            if (ret != 0)
            {
                throw std::runtime_error(format("RefineMesh fails with {}", ret));
            }
        ...
        std::string MeshTexturing(const std::string& mvs_name, const std::string& mesh_name, uint32_t max_texture_size)
        {
            ...
            if (ret != 0)
            {
                throw std::runtime_error(format("TextureMesh fails with {}", ret));
            }
