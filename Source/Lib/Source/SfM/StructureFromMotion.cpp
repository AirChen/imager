        ...
        SfM_Data IntrinsicAnalysis(const std::filesystem::path& image_dir) const
        {
            ...
            if (!parseDatabase(camera_sensor_db_path.string(), vec_database))
            {
                throw std::runtime_error(
                    "Invalid input database: " + camera_sensor_db_path.string() + ", please specify a valid file.");
            }
            ...
            for (auto iter = input_image_paths.begin(); iter != input_image_paths.end(); ++iter)
            {
                ...
                {
                    std::cerr << "Not have Image Header: " << image_file_path.string() << std::endl;
                    continue;
                }
                ...
                if (exif_reader.doesHaveExifInfo())
                {
                    ...
                    else
                    {
                        ...
                        {
                            const std::string cam_full_model = cam_brand + " " + cam_model;
                            ...
                            std::cerr << "Not have focal" << std::endl;
                        }
                    }
                } else {
                    std::cerr << "Not have exif Info: " << image_file_path.string() << std::endl;
                }
                ...
                if ((focal > 0) && (ppx > 0) && (ppy > 0) && (width > 0) && (height > 0))
                {
                    ...
                    std::cerr << "width: " << width << " height: " << height << " focal: " << focal << " ppx: " << ppx << " ppy: " << ppy << std::endl;
                }
    ...
    #pragma omp parallel for schedule(dynamic)
        ...
        Result ExportResult(const SfM_Data& sfm_data)
        {
            ...
            for (const auto& mvg_view : sfm_data.views)
            {
                ...
                {
                    throw std::runtime_error("Cannot read the corresponding image: " + src_image.string());
                }
