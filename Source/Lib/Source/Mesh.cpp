...
namespace AIHoloImager
{
    ...
    Mesh LoadMesh(const std::filesystem::path& path)
    {
        ...
        if (ai_scene)
        {
            ...
            {
                const float* vec_idx = &ai_mesh->mVertices[vi].x;
                const float* coor_idx = &ai_mesh->mTextureCoords[0][vi].x;
                mesh.Vertex(vi) = {{*vec_idx, *(vec_idx + 1), *(vec_idx + 2)}, {*coor_idx, *(coor_idx + 1)}};
            }
